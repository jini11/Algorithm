import java.util.*;

class Solution {
    
    int m, n, h, w;
    int[][] map, drops;
    
    public int[] solution(int m, int n, int h, int w, int[][] drops) {
        int[] answer = {0, 0};
        this.m = m;
        this.n = n;
        this.h = h;
        this.w = w;
        this.drops = drops;
        map = new int [m][n];
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                map[i][j] = drops.length;
            }
        }
        
        for (int i = 0; i < drops.length; i++) {
            if (map[drops[i][0]][drops[i][1]] == drops.length) {
                map[drops[i][0]][drops[i][1]] = i;
            }
        }
        int left = 0;
        int right = drops.length;
        while (left <= right) {
            int mid = (left + right) / 2;
            int[] zone = findZone(mid);
            if (zone != null) {
                answer = zone;
                left = mid + 1;
            } else {
                right = mid - 1;
            }
        }
        return answer;
    }
    
    private int[] findZone(int mid) {
        int[][] pSum = new int[m + 1][n + 1];
        
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                int val = (map[i][j] < mid) ? 1 : 0;
                pSum[i + 1][j + 1] = val + pSum[i][j + 1] + pSum[i + 1][j] - pSum[i][j];
            }
        }
        
        for (int i = h; i <= m; i++) {
            for (int j = w; j <= n; j++) {
                int sum = pSum[i][j] - pSum[i - h][j] - pSum[i][j - w] + pSum[i - h][j - w];
                if (sum == 0) return new int[]{i - h, j - w}; 
            }
        }
        return null;
    }
}